 ...
 * @LastEditTime: 2022-03-16 15:56:01
...
<template>
  <div class="right-small-box">
    <div class="box-title clearfix">
      ...
      <span class="float-r check-icon" @click="toDiagnosis"><i></i>薄弱词汇诊断</span>
    </div>
    <div class="main" v-show="isShow">
      <div class="left">
        ...
        <div class="scoreRate">
          <span>词汇平均答对率</span>
          <span class="rate"
            ><span class="number">{{ (info.VocabularyScoreRate * 100).toFixed(2) }}</span
            >%</span
          >
        </div>
      </div>
    ...
    >
      <div class="dialogContent">
        <div class="dialogMain">
          <el-table :empty-text="emptyText" :data="tableData" style="width: 450px">
            ...
            <el-table-column prop="VocaMasterCount" label="词汇掌握量" width="140">
              <template slot-scope="scope">
                <span class="gray">
            ...
            <el-table-column prop="VocaMaterRate" label="词汇答对率">
              <template slot-scope="scope">
                <span class="gray"> {{ (scope.row.VocaMaterRate * 100).toFixed(2) }}% </span>
              </template>
            </el-table-column>
            <template slot="empty">
...
<script>
import { GetClassVocabulary, GetClassVocaConclusionDetail } from "@/api/paperTwo/question.js";
import "echarts-gl";
export default {
  data() {
    return {
          ...
          formatter: (params) => {
            if (params.seriesName !== "mouseoutSeries") {
              return `${
                ...
                params.seriesName,
              )}</span>人 (${option.series[params.seriesIndex].pieData.value.toFixed(2)}%)`;
            }
          },
        },
        xAxis3D: {
...
.main {
  ...
  .left {
    ...
    .masteredCount {
      width: 201px;
      height: 74px;
      background: url(~@/assets/img/teacher/掌握1_iSpt.png) 0px center no-repeat;
      padding-left: 90px;
      .count {
        display: block;
        margin-top: 8px;
