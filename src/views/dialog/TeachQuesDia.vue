 ...
 * @LastEditTime: 2022-03-24 10:20:52
...
<template>
  <div class="DioContent">
    ...
    <div class="table" :style="{ height: getHeight(showList.length) }">
      <table class="ttable" style="display: inline-block; border: 0; vertical-align: top">
        <thead style="height: 65px; border-right: solid 1px rgba(225, 225, 225, 0.2)">
          <tr>
            <th>序号</th>
            <th>题型名称</th>
            <th>平均得分率</th>
      ...
      <vuescroll :ops="ops" style="width: calc(100% - 332px); display: inline-block">
        <!-- <vuescroll :ops="ops"> -->
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="(item, index) in showList" :key="index">
              <!-- <td>{{ item.QuesIndex }}</td>
							<td>{{ item.QTypeName }}</td>
							<td>{{ (item.PaperScoreRate * 100).toFixed() + "%" }}</td> -->
              <td class="borderTD" v-for="(item2, index2) in item.QTypeStuMapList" :key="index2">
