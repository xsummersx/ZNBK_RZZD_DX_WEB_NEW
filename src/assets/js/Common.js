...
function BsToCsFunc(initFunc, reciveExeMessageFunc, reConnectFunc) {
...
var _start = function (proName, moduleName, url, exeName, exeParam) {
  if (this.objState == 0) {
    if (this.objConnect) {
...
BsToCsFunc.prototype.startByFlag = function (flag, proName, moduleName, url, exeName, exeParam) {
  if (this.objState == 0) {
    if (this.objConnect) {
      var char1 = String.fromCharCode(1);
      var char2 = String.fromCharCode(2);
      //连接成功后
      this._Socket.send(proName + char1 + moduleName + char1 + "001" + char2 + flag + char2 + url + char2 + exeName + char2 + exeParam);
      return true;
    } else {
      return false;
    }
    ...
    if (this.objConnect) {
      this.activexObj.NotifyService(flag + url + "," + proName + "," + moduleName + "," + exeName + "#" + exeParam);
      return true;
    } else {
      return false;
    }
